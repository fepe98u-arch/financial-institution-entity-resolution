...
- 그래서 이 저장소를 GitHub(공개)에 올려도, 올라가는 것은 코드와 사용법뿐
  입니다. 실제 DB 접속 정보, APP_PASSWORD, 지금까지 쌓인 데이터는 전혀
  올라가지 않습니다. 다른 사람이 이 저장소를 내려받아도 자기 컴퓨터에
  PostgreSQL을 새로 설치하고 `.env`(DATABASE_URL, APP_PASSWORD)를 직접
  처음부터 만들어야만 실행할 수 있고, 그렇게 만든 화면은 내 컴퓨터에서
  지금 돌아가는 화면/데이터와는 완전히 별개입니다.
